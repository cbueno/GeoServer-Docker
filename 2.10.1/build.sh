...
docker build --build-arg TOMCAT_EXTRAS=false --build-arg GDAL_NATIVE=true -t cbueno/geoserver-docker .
## Note: disabling GWC may conflict with plugins in 2.9+ that have this as a dependency
#docker build --build-arg DISABLE_GWC=true --build-arg TOMCAT_EXTRAS=false -t cbueno/geoserver .
